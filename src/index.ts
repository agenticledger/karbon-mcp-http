#!/usr/bin/env node
/**
 * Karbon MCP Server — Streamable HTTP, BROKER-FIRST (auth model "B").
 *
 * This MCP holds ZERO Karbon secrets. It is a *client* of the Connections Broker
 * (https://connectionsbroker.agenticledger.ai), which vaults each user's Karbon
 ...
 *      `Authorization: Bearer <karbon-api-key>`, use it directly.
 *   2. Broker-first (default): derive the caller's `principal`, sign a short-lived
 *      JWT, ask the broker POST /token for the Karbon key, construct the client.
...
const SLUG = 'karbon';
const NAME = 'Karbon MCP Server';
const VERSION = '2.0.0';

/** Construct a KarbonClient from a broker-resolved (or raw passthrough) credential. */
function makeClient(credential: string): KarbonClient {
  const o = JSON.parse(credential); return new KarbonClient(o.accessKey ?? o.key, o.bearerToken ?? o.token);
}
...
app.get('/', (_req, res) => {
  res.json({
    ...
    version: VERSION,
    description:
      'Karbon Banking — accounts, transactions, recipients, payments, invoicing, and treasury operations through MCP tools.',
    mcpEndpoint: '/mcp',
    ...
    auth: {
      ...
      alternativeAuth: {
        ...
        description: 'Escape hatch (no secret held): pass a raw Karbon API key as Bearer.',
      },
    },
...
app.get('/health', (_req, res) => {
  res.json({
    ...
    authModes: [
      'broker-first (default): resolves the Karbon key via the Connections Broker',
      'bearer-passthrough (escape hatch): Authorization: Bearer <karbon-api-key>',
    ],
  });
});
...
type ClientResolution =
  | { kind: 'client'; client: KarbonClient }
...
async function connectMessage(principal: string): Promise<string> {
  ...
  if ('error' in started) {
    return `Karbon isn't connected for this caller yet, and starting a connection failed: ${started.error}`;
  }
  return JSON.stringify(
    {
      ...
      message:
        brokerProviderKind === 'static'
          ? 'Karbon is not connected for this caller yet. Connect it once via the link below (paste your Karbon API key into your platform’s broker connect flow), then run the tool again — it will work.'
          : 'Karbon is not connected for this caller yet. Open the connect link below once, then run the tool again — it will work.',
  ...
  if (raw) {
    ...
    if (!brokerConfigured) {
      res.status(503).json({
        ...
        alternative: { Authorization: 'Bearer <your-karbon-api-key>' },
      });
      return;
    }
