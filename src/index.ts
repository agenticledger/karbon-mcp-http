#!/usr/bin/env node
/**
 * Karbon MCP Server — Exposed via Streamable HTTP
 *
 * Auth model: Client sends their own Karbon credentials:
 *   - Authorization: Bearer <tenant-bearer-token>
 *   - X-Karbon-Access-Key: <access-key-jwt>
...
app.get('/', (_req, res) => {
  ...
  res.json({
    ...
    version: '1.0.0',
    description:
      'Access Karbon Practice Management — clients, contacts, work items, timesheets, and more through MCP tools.',
    mcpEndpoint: '/mcp',
    ...
    auth: {
      type: 'dual-mode',
      description:
        'Pass your Karbon Bearer token as Authorization header and Access Key as X-Karbon-Access-Key header. No credentials are stored on this server.',
      headers: {
        Authorization: 'Bearer <your-karbon-bearer-token>',
        'X-Karbon-Access-Key': '<your-karbon-access-key>',
      },
      howToGetKey:
        'Generate API credentials in Karbon: Settings > Connected Apps > API Keys',
    },
    configTemplate: {
      mcpServers: {
        karbon: {
    ...
    links: {
      health: '/health',
      documentation: 'https://financemcps.agenticledger.ai/karbon/',
    },
  });
});
      ...
      mcpServers: {
        karbon: {
          ...
          headers: {
            ...
            'X-Karbon-Access-Key': 'YOUR_ACCESS_KEY',
          },
        },
      },
    },
...
<head>
  ...
  <title>Karbon MCP Server — AgenticLedger</title>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    input[type="text"]{width:100%;padding:0.6rem 0.75rem;font-size:0.95rem;font-family:inherit;border:1px solid #cbd5e1;border-radius:8px;outline:none;transition:border-color .15s;margin-bottom:0.75rem}
...
<body>
  ...
  <div class="container">
    <h1>Karbon MCP Server</h1>
    <p class="subtitle">Access Karbon Practice Management — clients, contacts, work items, timesheets, and more through MCP tools.</p>

    <div class="card">
      <h2>How to get your Karbon API credentials</h2>
      <ol class="steps">
        <li>Log in to <a href="https://app.karbonhq.com" target="_blank" rel="noopener">Karbon</a>.</li>
        <li>Go to <strong>Settings &gt; Connected Apps &gt; API Keys</strong>.</li>
        <li>Generate an <strong>Access Key</strong> (JWT) and a <strong>Bearer Token</strong> (tenant key).</li>
    ...
    <div class="card">
      ...
      <label for="accessKey">Karbon Access Key (JWT)</label>
      <input type="text" id="accessKey" placeholder="Paste your Access Key here..." autocomplete="off" spellcheck="false" />
      <label for="bearerToken">Karbon Bearer Token</label>
      <input type="text" id="bearerToken" placeholder="Paste your Bearer Token here..." autocomplete="off" spellcheck="false" />
  ...
  <script>
    ...
    function buildConfig(accessKey, bearerToken) {
      return JSON.stringify({
        mcpServers: {
          karbon: {
            url: baseUrl + '/mcp',
            headers: {
              Authorization: 'Bearer ' + (bearerToken || 'YOUR_BEARER_TOKEN'),
              'X-Karbon-Access-Key': accessKey || 'YOUR_ACCESS_KEY'
            }
          }
        }
      }, null, 2);
    ...
    accessKeyInput.addEventListener('input', function () {
      configBlock.textContent = buildConfig(this.value.trim(), bearerTokenInput.value.trim());
    });
    bearerTokenInput.addEventListener('input', function () {
      configBlock.textContent = buildConfig(accessKeyInput.value.trim(), this.value.trim());
    });
...
function extractKarbonCredentials(req: express.Request): { accessKey: string; bearerToken: string } | null {
  const auth = req.headers.authorization;
  const accessKey = req.headers['x-karbon-access-key'] as string | undefined;

  if (!auth || !accessKey) return null;
...
function createMCPServer(client: KarbonClient): Server {
  const server = new Server(
    { name: 'karbon-mcp-server', version: '1.0.0' },
    { capabilities: { tools: {} } }
  );
...
app.post('/mcp', async (req, res) => {
  ...
  // New session — requires both credentials
  const credentials = extractKarbonCredentials(req);
  if (!credentials) {
    res.status(401).json({
      error: 'Missing credentials. Required headers: Authorization: Bearer <karbon-bearer-token>, X-Karbon-Access-Key: <access-key>',
    });
    return;
  }

  // Create per-session API client with the user's credentials
  const client = new KarbonClient(credentials.accessKey, credentials.bearerToken);
...
<head>
  ...
  <title>Authorize \u2014 Karbon MCP</title>
...
<body>
  <div class="card">
    <div class="header"><img src="/static/logo.png" alt="AgenticLedger"><span>Karbon MCP</span></div>
    <div class="consent-msg">An application wants to connect to <strong>Karbon MCP Server</strong> on your behalf. Enter your API key to authorize access.</div>
...
app.listen(PORT, () => {
  console.log(`Karbon MCP HTTP Server running on port ${PORT}`);
